...
class WebViewActivity : BaseActivity<ActivityWebViewBinding>(R.layout.activity_web_view) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){
            CookieManager.getInstance().setCookie(Constants.getWebViewHost(), Constants.jsessionId)
        }
...
//        ToastUtil.show(activity,"접속",3000)
    }
    ...
    fun openCamera() {
        cameraLauncher.launch(Intent(activity,CameraView::class.java))
    }
